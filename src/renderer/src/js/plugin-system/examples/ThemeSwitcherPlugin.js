...
class ThemeSwitcherPlugin extends PluginBase {
    ...
    addCustomTheme(themeId, themeData) {
        // 确保主题数据格式正确
        const theme = {
            name: themeData.name || `自定义主题 ${themeId}`,
            dataTheme: 'custom',
        ...
        this.updateThemePanel();
        console.log(`🎨 ThemeSwitcherPlugin: 添加自定义主题: ${themeId}`);
    }
    ...
    removeCustomTheme(themeId) {
        if (themeId === 'light' || themeId === 'dark') {
            console.warn('🎨 ThemeSwitcherPlugin: 不能移除内置主题');
            return;
        }

        delete this.themes[themeId];

        // 如果当前使用的是被删除的主题，切换到浅色主题
        if (this.currentTheme === themeId) {
            this.applyTheme('light');
        }

        this.updateThemePanel();
        console.log(`🎨 ThemeSwitcherPlugin: 移除自定义主题: ${themeId}`);
    }
    ...
    exportCurrentTheme() {
        ...
        a.href = url;
        a.download = `${this.currentTheme}-theme.json`;
        a.click();
        
        URL.revokeObjectURL(url);
        
        this.showNotification('主题已导出', 'success');
    }
    ...
    async importTheme(themeData) {
        try {
            ...
            if (!theme.id || !theme.name || !theme.colors) {
                throw new Error('主题格式无效，需要包含id、name和colors字段');
            }

            // 验证颜色格式
            const requiredColors = ['color-body-bg', 'color-text', 'color-primary'];
            const hasRequiredColors = requiredColors.some(color => theme.colors[color]);

            if (!hasRequiredColors) {
                throw new Error('主题颜色格式无效，至少需要包含基本颜色变量');
            }

            this.addCustomTheme(theme.id, {
                name: theme.name,
                colors: theme.colors
            });

            this.showNotification(`主题 ${theme.name} 导入成功`, 'success');

        } catch (error) {
            console.error('🎨 ThemeSwitcherPlugin: 导入主题失败:', error);
    ...
    getMetadata() {
        return {
            ...
            customThemes: Object.keys(this.themes).filter(id =>
                !['light', 'dark'].includes(id)
            ).length
        };
    }
    ...
    getStatus() {
        return {
            ...
            currentTheme: this.currentTheme,
            themeName: this.themes[this.currentTheme]?.name || '未知主题',
