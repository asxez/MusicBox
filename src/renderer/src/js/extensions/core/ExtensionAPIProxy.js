...
import {Permissions} from "@extensions/core/ExtensionPermissions";
 ...
 * @param {PermissionManager} permissionManager - 权限管理器
 * @param {string} namespace - API 命名空间（如 'player', 'library'）
 * @returns {Proxy} 代理后的 API 对象
 */
export function createAPIProxy(api, extensionId, permissionManager, namespace = '') {
...
/**
 * 创建完整的扩展 API 代理
 * @param {Object} fullAPI - 完整的 API 对象
 * @param {string} extensionId - 扩展 ID
 * @param {PermissionManager} permissionManager - 权限管理器
 * @returns {Object} 代理后的 API 对象
 */
export function createExtensionAPIProxy(fullAPI, extensionId, permissionManager) {
    const proxiedAPI = {};
...
export class APICallLogger {
    constructor() {
        this._logs = new Map(); // extensionId -> calls[]
        this._maxLogsPerExtension = 1000;
    }

    /**
     * 记录 API 调用
     */
    log(extensionId, apiPath, args, result, error = null) {
 ...
 * 创建带日志记录的 API 代理
 */
export function createLoggingAPIProxy(api, extensionId, permissionManager, logger, namespace = '') {
    if (!api || typeof api !== 'object') {
        return api;
    }
            ...
            if (typeof value === 'function') {
                return async function (...args) {
                    ...
                    try {
                        ...
                        // 记录失败调用
                        logger.log(extensionId, apiPath, args, null, error);
                        throw error;
                    }
                };
            }
