 ...
 * 全局快捷键注册表（局部快捷键）
 */
const localKeybindingsRegistry = new Map();

/**
 * 全局快捷键注册表（全局快捷键）
 */
const globalKeybindingsRegistry = new Map();

/**
 * 插件全局快捷键配置映射 (用于与系统快捷键合并)
 * Map<shortcutId, {id, name, description, key, enabled}>
 */
const extensionGlobalShortcutsConfig = new Map();

/**
 * 快捷键监听器
 */
let keybindingListener = null;
 ...
 * @returns {KeybindingsAPI} 快捷键 API 实例
 */
export function createKeybindingsAPI(context) {
    return {
        /**
         * 注册局部快捷键
         * @param {string} keybinding - 快捷键组合（如 'Ctrl+Shift+P'）
         * @param {Function} callback - 回调函数
         * @param {Object} [options={}] - 选项
         * @param {string} [options.commandId] - 关联的命令ID
         * @param {string} [options.when] - 上下文条件
         * @param {string} [options.description] - 描述
         * @returns {Disposable} 可释放对象
         */
        async registerKeybinding(keybinding, callback, options = {}) {
        ...
        /**
         * 注册全局快捷键
         * @param {string} keybinding - 快捷键组合（如 'Alt+Ctrl+P'）
         * @param {Function} callback - 回调函数
         * @param {Object} [options={}] - 选项
         * @param {string} [options.commandId] - 关联的命令ID
         * @param {string} [options.description] - 描述
         * @returns {Disposable} 可释放对象
         */
        async registerGlobalKeybinding(keybinding, callback, options = {}) {
            return registerKeybindingInternal(
                keybinding,
                callback,
        ...
        /**
         * 获取所有已注册的快捷键
         * @param {string} [scope] - 作用域过滤（可选）
         * @returns {Array<Object>} 快捷键列表
         */
        getKeybindings(scope) {
            if (scope !== undefined) {
                Validator.assertEnum(scope, Object.values(KeybindingScope), 'scope');
            }
         ...
         */
        hasKeybinding(keybinding, scope = KeybindingScope.LOCAL) {
            Validator.assertNonEmptyString(keybinding, 'keybinding');
            Validator.assertEnum(scope, Object.values(KeybindingScope), 'scope');
         ...
         * @param {string} keybinding - 快捷键组合
         * @param {string} [scope='local'] - 作用域
         * @returns {Object|null} 快捷键信息
         */
        getKeybindingInfo(keybinding, scope = KeybindingScope.LOCAL) {
            Validator.assertNonEmptyString(keybinding, 'keybinding');
            Validator.assertEnum(scope, Object.values(KeybindingScope), 'scope');
         ...
         */
        async triggerKeybinding(keybinding, scope = KeybindingScope.LOCAL) {
            Validator.assertNonEmptyString(keybinding, 'keybinding');
            Validator.assertEnum(scope, Object.values(KeybindingScope), 'scope');
 ...
 * @returns {Disposable} 可释放对象
 */
async function registerKeybindingInternal(keybinding, callback, scope, context, options = {}) {
    Validator.assertNonEmptyString(keybinding, 'keybinding');
    ...
    return await ErrorUtils.wrapAsync(async () => {
        ...
        if (scope === KeybindingScope.GLOBAL) {
            ...
            window.addEventListener('globalShortcutTriggered', eventHandler);
        ...
        return toDisposable(async () => {
            ...
            if (scope === KeybindingScope.GLOBAL) {
                ...
                const shortcutId = options.id || Array.from(extensionGlobalShortcutsConfig.entries())
                    .find(([id, config]) => config.key === normalizedKey)?.[0];
                ...
                if (shortcutId) {
                    ...
                    if (keybindingInfo.globalEventHandler) {
                        window.removeEventListener('globalShortcutTriggered', keybindingInfo.globalEventHandler);
                    }
...
async function syncGlobalShortcuts() {
    try {
        ...
        // 合并系统快捷键和插件快捷键
        const allShortcuts = {...systemShortcuts};
...
/**
 * 获取系统全局快捷键配置
 * @returns {Promise<Object>} 系统快捷键配置
 */
async function getSystemGlobalShortcuts() {
 ...
 * 初始化局部快捷键监听器
 */
function initializeLocalKeybindingListener() {
    if (keybindingListener) {
        return;
    }

    keybindingListener = (e) => {
        // 如果焦点在输入框中，不处理快捷键
...
/**
 * 检查是否应该执行快捷键（基于上下文条件）
 * @param {Object} keybindingInfo - 快捷键信息
 * @returns {boolean} 是否应该执行
 */
function shouldExecuteKeybinding(keybindingInfo) {
    ...
    switch (when) {
        ...
        case KeybindingWhen.LYRICS_VISIBLE:
            return document.querySelector('.lyrics-panel')?.style?.display !== 'none';
        default:
            return true;
    }
}
 ...
 * 清理快捷键监听器
 */
export function disposeKeybindingListener() {
    if (keybindingListener) {
